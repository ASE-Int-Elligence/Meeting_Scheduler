...
class Home_page(object):
	...
	def show_group_details(self, key):
		...
		if self.root.userid == self.groups_info[key]["admin"]:
			...
			self.user_name_entry.place( relx = 0.5, rely = 0.8, anchor = CENTER)

			add_user_button = Button(self.top, text="Search User", command = self.create_popup)
			add_user_button.place( relx = 0.8, rely = 0.8, anchor = CENTER)

			self.save_changes = Button(self.top, text = "Save Changes", command = lambda p = key: self.edit_group(p))
			self.save_changes.place( relx = 0.5, rely = 0.9, anchor = CENTER)
	...
	def load_groups_info(self):
		...
		for key in self.groups_info.keys():
			...
			left = Label( self.label_frames[-1], width = 50, text = "Group type : "+group_info["groupType"] )
			...
			self.create_meeting_buttons.append( Button( self.label_frames[-1], text = "Create Meeting",
					command = lambda arg = group_info["groupID"]: Create_meeting.meeting_tab( arg, self) ))
			self.create_meeting_buttons[-1].place( relx = 0.5, rely = 0.5, anchor = CENTER)
